...
export interface TeamMember { id: string; name: string; email: string; role: Role; isYou: boolean }
...
export async function getTeam(): Promise<TeamData> {
  ...
  try {
    ...
    const [{ data: profiles }, { data: invites }] = await Promise.all([
      ...
      supabase.from('invites').select('id, email, role, token, expires_at').eq('company_id', companyId).is('accepted_at', null).order('created_at', { ascending: false }),
    ])
