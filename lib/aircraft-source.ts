...
const traceCache = new Map<string, { at: number; raw: unknown | null }>()
...
export async function fetchTraceDay(hex: string, day: string, now = new Date()): Promise<unknown | null> {
  ...
  try {
    ...
    if (traceCache.size > 400) traceCache.clear()
    traceCache.set(key, { at: Date.now(), raw })
    return raw
  } catch {
...
export async function identFromTrace(hex: string, now = new Date()): Promise<AircraftIdent | null> {
  for (const day of availableDays(now, 5)) {
