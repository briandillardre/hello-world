...
export const TASKS: BoardTask[] = [
  ...
  { id: 119, title: 'No Play background-location declaration needed after all — the shift recorder is a foreground service; v1.4.1 went straight to production', why: 'v1.4.0 declared Android’s background-location permission, which would have parked the release behind Play’s declaration-and-video review. The reviewer pass the same night showed the plugin never asks for it: a location foreground service (the persistent “on the clock” notification) keeps recording with the screen off on plain “While using the app” permission. 1.4.1 (versionCode 10) drops the permission, adds the bridge setting the plugin needs to keep delivering past 5 minutes in the background, and was dispatched to production. Crew phones need the update from the Play Store to get screen-off recording; until then the app records while it is open.', owner: 'build', state: 'done' },
