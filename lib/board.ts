...
export const TASKS: BoardTask[] = [
  ...
  { id: 41, title: 'Apple: enroll as an Individual (org route DENIED Sep 4)', why: 'Apple denied the HAMMERTRACK LLC organization enrollment as final (phone, Sep 4 — Megan; case 20000149520723). Decision: go personal. Apple Developer app on the iPhone → Account → Enroll → Individual → scan driver’s license + selfie → $99 on the Mercury card. No D-U-N-S, no LLC papers. Seller shows “Brian Dillard”; Apple converts to Organization later on request. Approval day (~15 min): App Store Connect → Users and Access → Integrations → generate a TEAM API key with the Admin role, download the .p8 once, copy the Key ID + Issuer ID, read the Team ID from Membership → four GitHub secrets (ASC_KEY_ID, ASC_ISSUER_ID, ASC_KEY_P8, ASC_TEAM_ID) → say “apple secrets are in” and the ios-testflight workflow builds on GitHub’s Macs and lands in TestFlight the same hour. Everything else is ready as of Sep 10: signing, 1.4.1, permission strings, review notes, demo account, in-app login. Runbook: docs/APP-STORE-PLAYBOOK.md → Approval day.', owner: 'brian', state: 'open', sev: 'warn' },
