...
export const TASKS: BoardTask[] = [
  ...
  // ── Yours, soon ──────────────────────────────────────────────────────
  { id: 119, title: 'Play Console: fill in the Location permissions declaration for v1.4.0 (background shift tracking)', why: 'The new app build records a clocked-in shift with the screen off, which needs Android’s “allow all the time” location permission. Google will not release a build with that permission to production until Play Console → App content → Location permissions is answered (use case: employee time tracking while clocked in; the in-app disclosure is the sheet that appears at the first clock-in) and a short screen recording of that flow is attached — the wording is ready in docs/APP-STORE-PLAYBOOK.md under “Background location”. Until then the build sits on the internal testing track; production is one workflow dispatch after approval.', owner: 'brian', state: 'open' },
