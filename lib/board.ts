...
export const TASKS: BoardTask[] = [
  ...
  // ── Yours, soon ──────────────────────────────────────────────────────
  { id: 119, title: 'Play Console: answer the Foreground service permissions declaration so app 1.4.1 (the clocked-in shift recorder) can go live', why: 'The 1.4.1 build uploaded fine but Play refused to release it: “You must let us know whether your app uses any Foreground Service permissions.” It is a one-time Android-14 form — much lighter than the background-location review we avoided by recording through a foreground service. Play Console → App content → Foreground service permissions → type Location: describe the feature (while an employee is clocked in, the app records the phone’s location to their time card and the crew map; runs only between clock-in and clock-out; shows a persistent notification) and attach a 30–60 s screen recording of clock-in → disclosure → notification → clock-out. Exact wording is in docs/APP-STORE-PLAYBOOK.md. **Sep 12 — why you could not find the form:** App content showed ten actioned declarations and none of them was this one. Play only offers the form once a build that uses the permission is actually sitting in the console, and the failed release took the uploaded build down with it (Play saves the upload and the release together, or not at all) — so each was waiting on the other. Broken from this side: the 1.4.1 build is now **parked in the console as a draft** production release (uploaded 06:09 UTC Sep 12) — it publishes nothing, and it is what makes the form appear. **Two taps when it does:** Monitor and improve → Policy → App content → Foreground service permissions (type Location, paste the wording, attach the clip) — then open the waiting draft under Production and hit **Start rollout**. Same build, nothing to rebuild.', owner: 'brian', state: 'open' },
