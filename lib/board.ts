...
export const TASKS: BoardTask[] = [
  ...
  // ── Yours, soon ──────────────────────────────────────────────────────
  { id: 99, title: 'Get the free CARTO basemaps key — the map is stamped “API KEY REQUIRED”', why: 'CARTO now requires a key for its raster basemaps; the Command Center’s dark base and the Streets/Silver/Plain styles carry an “API KEY REQUIRED” watermark on every tile. Free up to 5M tiles a month: carto.com/basemaps/apikey (email + domain hammertrack.ai, one minute, no account) → paste as NEXT_PUBLIC_CARTO_KEY in Vercel → redeploy. The code already sends it.', owner: 'brian', state: 'open', sev: 'stop' },
...
export const ENV_PENDING: { text: string; tone: 'no' | 'wait' }[] = [
  ...
  { text: 'NEXT_PUBLIC_CARTO_KEY — until set, CARTO basemap tiles carry an “API KEY REQUIRED” stamp (board #99)', tone: 'no' },
