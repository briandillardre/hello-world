...
export default function ProfileScreen() {
  ...
  if (!profile) {
    return (
      ...
      >
        ...
        <Text style={styles.setupDisclaimer}>Stored locally. No account needed.</Text>
      </KeyboardAvoidingView>
    );
  }
  ...
  const gameBreakdown = Object.values(GAMES_INFO).map(g => {
    const plays = profile.scores.filter(s => s.gameId === g.id);
    const best = plays.length ? Math.max(...plays.map(s => s.score)) : 0;
    return { ...g, plays: plays.length, best };
  });
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.header}>
        ...
        <View>
          ...
          <Text style={styles.joinDate}>Royalty since {new Date(profile.joinDate).toLocaleDateString()}</Text>
        </View>
      </View>
          ...
          {myKingdoms.map(ra => (
            <View key={ra.id} style={styles.kingdomCard}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.kingdomSub}>{ra.highway} · {ra.state}</Text>
              </View>
      ...
      {gameBreakdown.map(g => (
        <View key={g.id} style={[styles.gameStatCard, { borderLeftColor: g.color }]}>
          ...
          <View style={{ alignItems: 'center' }}>
            <Text style={{ color: '#64748b', fontSize: 10 }}>Best</Text>
        ...
        <>
          ...
          {recentScores.map((s, i) => (
            <View key={s.id ?? i} style={styles.scoreRow}>
              <Text style={{ fontSize: 20 }}>{GAMES_INFO[s.gameId]?.emoji ?? '🎮'}</Text>
              <View style={{ flex: 1 }}>
                <Text style={styles.scoreGame}>{GAMES_INFO[s.gameId]?.name ?? s.gameId}</Text>
                <Text style={styles.scoreTimestamp}>{new Date(s.timestamp).toLocaleDateString()}</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  nameInput: {
    backgroundColor: '#1e1e3a', borderRadius: 12, padding: 16,
    color: '#f1f5f9', fontSize: 18, fontWeight: '600', width: '100%',
    borderWidth: 1, borderColor: '#4a4a8a', textAlign: 'center',
  },
  claimBtn: { backgroundColor: '#7c3aed', borderRadius: 14, paddingVertical: 16, paddingHorizontal: 40, width: '100%', alignItems: 'center' },
  claimBtnDisabled: { backgroundColor: '#2d2d5a' },
  claimBtnText: { color: 'white', fontWeight: '800', fontSize: 16 },
  setupDisclaimer: { color: '#4a5568', fontSize: 12 },
  header: { flexDirection: 'row', alignItems: 'center', gap: 14, marginBottom: 20 },
  avatarCircle: { width: 60, height: 60, borderRadius: 30, backgroundColor: '#7c3aed', alignItems: 'center', justifyContent: 'center' },
  ...
  statCard: {
    flex: 1, minWidth: '45%', backgroundColor: '#1e1e3a', borderRadius: 12,
    padding: 14, alignItems: 'center', gap: 4, borderWidth: 1, borderColor: '#2d2d5a',
  },
  ...
  kingdomCard: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: '#1e1e3a', borderRadius: 10,
    padding: 12, marginBottom: 8, gap: 10, borderWidth: 1, borderColor: '#f59e0b44',
  },
  kingdomName: { color: '#f1f5f9', fontSize: 14, fontWeight: '600' },
  kingdomSub: { color: '#64748b', fontSize: 12, marginTop: 2 },
  kingdomScore: { color: '#f59e0b', fontSize: 16, fontWeight: '700' },
  gameStatCard: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: '#1e1e3a',
    borderRadius: 10, padding: 12, marginBottom: 8, gap: 10, borderLeftWidth: 3,
  },
  ...
  scoreRow: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: '#1e1e3a',
    borderRadius: 10, padding: 10, marginBottom: 6, gap: 10,
  },
