...
export default async function WelcomePage() {
  ...
  const steps: { key: string; done: boolean; icon: typeof Plus; title: string; body: string; note?: string; href: string; cta: string }[] = [
    ...
    { key: 'tracker', done: reporting, icon: Radio, title: 'Plug in a tracker', body: 'Create the asset with the tracker ID (IMEI) first, then plug the unit in — its first report puts it live on your map.', note: 'Hardware ships in batches — while yours is in the mail, add your team and draw your first zone. Pushing data from your own hardware? Your company API key and the endpoints are in the integration section.', href: '/settings#integration', cta: 'What happens when trackers arrive' },
    { key: 'zone', done: hasZone, icon: Hexagon, title: 'Draw a site zone', body: 'Outline your yard, a job site or a customer property so theft alerts and cost-per-site kick in.', href: '/map', cta: 'Open the map' },
