...
import { featureLabel, isProspect, PROSPECT_NEVER, PROSPECT_HIDDEN, FEATURE_KEYS, type FeatureKey } from '@/lib/permissions'
 ...
 */
export default async function LockedPage({ searchParams }: { searchParams: { f?: string } }) {
  const perms = await getMyPermissions()
  const key = (FEATURE_KEYS as string[]).includes(searchParams.f ?? '') ? (searchParams.f as FeatureKey) : null
  if (!isProspect(perms) || !key || perms.features.includes(key) || PROSPECT_HIDDEN.includes(key)) redirect('/map')
  const label = featureLabel(key) ?? 'This page'
      ...
      <div className="max-w-xl mx-auto px-4 py-10">
        <div className="rounded-2xl border border-navy-800 bg-navy-900 p-6">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-3">
            <Link href="/map" className="inline-flex items-center gap-1.5 rounded-lg bg-amber text-[#1a1100] font-display font-bold text-[13px] px-3.5 py-2 shadow-glow-amber hover:brightness-110 transition">
