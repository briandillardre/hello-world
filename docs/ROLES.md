...
* **Changes nothing.** A restrictive read-only policy for the prospect role on every RLS-enabled public table (insert / update / delete), so a direct PostgREST call with their JWT cannot write either. Every push switch defaults off. The photo actions (service-role writes) refuse the role with a plain sentence.
* **Sees the product the way the owner does (Sep 23 — Brian, of a preview that showed only Live Map / Assets / Zones: "this is not acceptable. I need to have the 'prospective client' able to be set to each level and they should see what I see — command center, all buttons, but the ones they don't have access to should show some 'sorry, you do not have access to this' or something, but they should not see team or financials or be seen by anyone else on the team except for me").** Their view levels default to everything the role CAN hold — `map` · `command` · `alerts` · `aircraft` · `assets` · `zones` · `measurements` · `maintenance` · `reports` — and the Master narrows per company in the view-levels table (the prospect column has a real switch for each of those). `PROSPECT_NEVER` (edit · $ figures · billing · manage team · Ask AI · team · activity · finance · clock · logs · share location · tags · receipts · accounting · settings · trackers · hardware) is still stripped LAST in `resolvePermissions` and shows as "—" in the table. **The difference from every other role is what OFF looks like.** For everyone else a page outside their view levels does not exist (absent from the navs, 404). For a prospect it is **LOCKED**: `navStateFor(path, features, role)` in `lib/permissions.ts` returns `open` · `locked` · `hidden`, and the three navs (sidebar, phone bar + More drawer, the map's account menu) draw a locked entry dimmed with a lock, leading to **`/locked?f=<key>`** — a page that says "Sorry — you don't have access to this", names the feature, and says whether the owner can switch it on (a grantable key) or it is simply not part of a Prospective Client login (a NEVER key: the crew's and the office's side). `requireFeature` sends a prospect there instead of a 404, so a typed URL lands on the same card. The phone bar seats only OPEN pages (locked ones live in the drawer). The Ask AI button stays (the panel shows the API's 403 sentence instead of a run). The exception inside the exception: **`PROSPECT_HIDDEN`** — `team` · `activity` · `finance` — stays absent from every nav and 404s, because the people and the money must not even be hinted at. **Ship-check follow-up the same night:** the Command Center's own sidebar and phone bar never took the view levels (harmless while no prospect could open the wall; with `command` on by default a prospect saw Team / Financials / Team activity as open tiles there) — `CommandCenter` now takes `features` and the same `askAi` rule; alert acknowledgement hides where it cannot persist (a role without `edit`, a prospect, a preview — the optimistic paint used to show theft as handled until the next reload) and the two ack actions refuse a preview or a prospect; `/api/insights` answers a prospect with nothing (the table is outside their allow-list and the route reads with the service role — the wall and the Ask AI chips ask for it now) and refuses dismissal from a preview; the locked page names the page you tapped (`&p=` for Time cards / Photos) and its home button goes to the FIRST open page, never back to a lock; the photo writers refuse a view-as preview like `movePhotoAction` already did; the phone drawer shows a prospect no My phone button.
...
* **Ask AI answers a session only, and only about what the asker may see (Sep 24 reviewer pass).** Both doors of `/api/assistant` answer 401 without a signed-in user (the permission resolver alone let a signed-out caller spend model credits), the POST is rate-limited per device (20/min), and an admin sees a plain category when it falls back ("out of credits", "key refused") — never the provider's raw error. The shared registry tools (`find_tool`, `maintenance_status`) read with the service role, so Ask AI passes the asker's own asset list (`visibleAssetIds` — RLS-read, view-as filtered by `scopeFleet`) and a tag, truck or service schedule outside it stays hidden. The company-key MCP door is unchanged (admin-grade).

## Money and acknowledgements (Sep 24 reviewer pass)

* **No dollars in a page payload without the costs level.** `scopeFleet()` in `lib/permissions.ts` is the one cut every fleet-carrying page makes before props reach the browser: the assets the viewer may see (111, so a "view as" preview is honest), the tag pairings whose BOTH ends they may see, alerts about machines they may see, and `withoutCosts()` blanking $/day, rates, purchase price and value. `/api/map-data` did this since Aug 12; the Command Center, Alerts and Ask AI now do too, and /reports keeps its Billable value tile (and the number) off the page without `costs`.
* **Acknowledging an alert takes the edit level.** It clears the alert for the whole company; the two ack actions refuse without `edit`, in a preview, or for a prospect (the UI already hid the buttons). The old acknowledge-everything action had no gate at all and no caller — deleted.
...
* **The view-levels table cannot widen a prospect past the sandbox.**
  ...
  activity · finance · clock · logs · share location · tags · receipts ·
  accounting · settings · trackers · hardware — Command Center left the list
  on Sep 23) is stripped LAST in `resolvePermissions`, like the master-only
  keys; the table shows those cells as "—". The Master may still narrow what
  a prospect sees of the rest (see *Sees the product the way the owner does*
  above).
