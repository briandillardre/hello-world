...
## Talking to the app (v1.5.2 / versionCode 13, Sep 19)

Brian, Sep 19: *"Talk to ai verbally button not working either."* The Android
WebView exposes `webkitSpeechRecognition` but has NO recognition service
behind it: the mic button showed, `start()` failed straight into `onerror`,
and the button blinked for a frame. So voice inside the app is native:
`@capacitor-community/speech-recognition` (7.0.1, peer `@capacitor/core >=7`,
Java 21 — builds against Capacitor 8) wraps Android's `SpeechRecognizer`.
`lib/speech-input.ts` is the one door: browser recognizer on the web, the
plugin in the shell, and on an app build from before the plugin the mic
STAYS and says on tap that it needs the update (never a button that does
nothing). Ask AI and the map's search box both use it.

- **Permission:** `RECORD_AUDIO` — a plain runtime permission, no Play
  declaration form. The plugin's manifest declares it (plus the
  `RecognitionService` query); ours repeats it so the intent is visible.
- **Release:** versionCode 13 sits behind the SAME Photo and Video
  Permissions declaration as 1.5.1 (the manifest still carries
  `READ_MEDIA_IMAGES`), so it is dispatched `status: draft` like 11 and 12
  and `play-promote` rolls out whichever draft is newest once Brian files
  the form. Play keeps one draft per track: a newer draft supersedes the
  older one, nothing is lost.
- **iOS:** the plugin ships a podspec but no `Package.swift`, and our iOS
  project is SPM-only (`ios/App/CapApp-SPM/Package.swift`, regenerated by
  `cap sync ios`). It is deliberately NOT added there — the iOS build would
  fail on a missing package. When iOS ships: add it through CocoaPods, or
  write a ~100-line local plugin over `SFSpeechRecognizer` (the
  OriginalPhotosPlugin pattern) and add `NSSpeechRecognitionUsageDescription`
  + `NSMicrophoneUsageDescription` to Info.plist. Until then iOS reads the
  "needs the update" state, which is honest.
...
4½. **Voice** — ✅ Android, v1.5.2 (Sep 19): Ask AI + map search talk through the native recognizer; iOS waits on the plugin's missing SPM package (above).
