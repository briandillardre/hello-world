...
- **Palette:** asphalt charcoal `#16181A` / warm paper `#F7F5F1` / **safety amber `#E8A020`** accent. Reads jobsite-authentic and photographs well against real asphalt/dirt imagery.
- **Logo:** current mark unseen (Wix blocked); the site ships with a clean typographic lockup + amber chevron ("A-frame/mountain") placeholder that can be replaced by the real logo file in one CSS/HTML swap. If the current logo is weak, this lockup is usable as-is.
