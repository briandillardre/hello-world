...
import { type Project, periodCost, lifetimeBurn, money, moneyFull, STATUS_META, RANGE_COST_LABEL } from '@/lib/projects'
    ...
    <div className="absolute right-3 top-[150px] z-10 w-[236px] rounded-xl bg-navy-950/85 backdrop-blur border border-navy-700 shadow-panel overflow-hidden">
      <button onClick={() => setOpen((v) => !v)} className={`w-full flex items-center justify-between px-3 py-2.5 ${open ? 'border-b border-navy-800' : ''}`}>
        <span className="font-mono text-[10px] uppercase tracking-[0.12em] text-faint flex items-center gap-2 min-w-0">
          ...
          <span className="text-amber normal-case tracking-normal truncate">{money(totalLabor + totalEquip)} · {periodLabel}</span>
        </span>
        {open ? <ChevronUp className="h-4 w-4 text-faint flex-none" /> : <ChevronDown className="h-4 w-4 text-faint flex-none" />}
      </button>
      ...
      {open && (
        <div className="p-2.5 space-y-2.5">
          {projects.map((p, i) => {
            const c = costs[i]
            const burn = lifetimeBurn(p)
            const st = STATUS_META[burn.status]
            return (
              <div key={p.id} className="rounded-lg border border-navy-800 bg-navy-900 p-2.5">
                ...
                {/* budget burn (lifetime) */}
                <div className="mt-2 h-1.5 rounded-full bg-navy-800 overflow-hidden">
                  <div
                    className="h-full rounded-full"
                    style={{ width: `${Math.min(100, burn.pct)}%`, background: burn.status === 'over_budget' ? '#fb5d5d' : p.color }}
                  />
                </div>
                <div className="mt-1 font-mono text-[10px] text-faint">
                  {moneyFull(p.spentToDate)} of {money(p.budget)} · {burn.pct.toFixed(0)}%
                </div>
                ...
                <div className="mt-2 grid grid-cols-2 gap-1.5">
                  <div className="rounded-md bg-navy-950/60 px-2 py-1.5">
                    ...
                    <div className="font-display font-bold text-[13px] text-ink">{money(c.labor)}</div>
                  </div>
                  <div className="rounded-md bg-navy-950/60 px-2 py-1.5">
                    <div className="font-mono text-[9px] text-faint flex items-center gap-1"><Wrench className="h-3 w-3" /> Equip</div>
                    <div className="font-display font-bold text-[13px] text-ink">{money(c.equip)}</div>
                  </div>
                </div>
              </div>
            )
          ...
          <div className="rounded-lg border border-amber/30 bg-amber/[0.06] p-2.5">
            <div className="font-mono text-[9px] uppercase tracking-[0.1em] text-faint">Cost · {periodLabel}</div>
            <div className="font-display font-black text-[20px] text-amber">{moneyFull(totalLabor + totalEquip)}</div>
            <div className="font-mono text-[10px] text-faint mt-0.5">
              {money(totalLabor)} labor + {money(totalEquip)} equipment
            </div>
          </div>
        </div>
      )}
