...
import {
  ...
  trailSegmentsSpeed, SPEED_CLASS_COLORS, SPEED_CLASS_MPH, sparseHops,
...
import { STACK_RADIUS_PX, STACK_MAX_ZOOM, STACK_SOURCE_MAXZOOM, FAN_MAX, stackMove, fanLayout, fanExtent, nudgeInto, centroidOf, fanStillHolds, nearbyIds, stackRadiusMetres, type StackPoint } from '@/lib/map-stacks'
...
import type { LinkState } from './ExportMenu'
import { defaultViewTitle, type SharedView } from '@/lib/share-links'
import type { OpenedViewLink } from '@/lib/db/share-links'
...
function trailsGeoJSON(tracks: AssetTrack[], filter: Set<AssetType>, t: number, selId?: string | null, speedWindowSec?: number | null): GeoJSON.FeatureCollection {
  // Tools draw NO trail line of their own (Brian, Aug 30: parallel colored
  // chords along his drive were three tags re-drawing the truck's path).
  // Their path IS the carrier's path — the carrier draws it once; tools
  // keep their moving replay head.
  const lineTracks = tracks.filter((tr) => filter.has(tr.type) && tr.type !== 'tool')
  // Sparse hops render as dashed "went roughly this way" connectors — never
  // solid ink (a 5-min-cadence tracker at 65 mph is a 5-mile straight chord).
  const sparse: GeoJSON.Feature[] = lineTracks.flatMap((tr) =>
    sparseHops(tr, t).map((h) => ({
      type: 'Feature' as const,
      geometry: { type: 'LineString' as const, coordinates: h.seg },
      properties: {
        id: tr.assetId,
        sparse: 1,
        color: speedWindowSec ? SPEED_CLASS_COLORS[speedClassOf(h.mph ?? 0)] : tr.color,
        fade: 1,
        sel: selId === tr.assetId ? 1 : 0,
        dim: selId && selId !== tr.assetId ? 1 : 0,
      },
    })))
...
/** SPEED_CLASS_MPH bucket for a single hop's mph. */
function speedClassOf(mph: number): number {
  for (let i = 0; i < SPEED_CLASS_MPH.length; i++) if (mph < SPEED_CLASS_MPH[i]) return i
  return SPEED_CLASS_MPH.length
}
...
function pointsGeoJSON(tracks: AssetTrack[], filter: Set<AssetType>, t: number, selId?: string | null, windowSec = 86_400): GeoJSON.FeatureCollection {
  ...
  for (const tr of tracks) {
    ...
    // Tool paths clone the carrier's — heat-weighting them re-counts the
    // same dwell once per tag aboard (see the matching skip in heat3d).
...
interface MapViewProps {
   ...
   *  applied once, when the map is ready. */
  sharedView?: OpenedViewLink | null
  /** Division filter (106) — owned by MapPageClient. The share sheet
   *  snapshots it and an opened shared view sets it. */
  divisionFilter?: string | null
  onDivisionFilter?: (v: string | null) => void
}
...
export function MapView({ assets, geofences, places = [], onPlacesChanged, tracks = [], historyRows = null, siteOverlays = [], earliestMs = null, tz = 'America/New_York', toolGateways, aboard, pairingEpisodes, onGeofenceSave, onGeofenceEdit, onGeofenceDelete, alerts = [], focusMeasurement = null, measurements = [], kiosk = false, tourOn = true, onTourInterrupt, defaultWeatherPlace = null, defaultWeatherCoords = null, canViewCosts = true, canFlightLog = false, savedMapViews = null, onSaveMapViews, brand = null, sharedView = null, divisionFilter = null, onDivisionFilter }: MapViewProps) {
  ...
  // one, fixed when it opens so it never flips under a finger). `openZ`: the
  // whole zoom it opened at — clusters are cut per whole zoom, so a pinch
  // past it folds the fan (the members split into their own pucks, or merge
  // into a bigger count, and a fan left open would draw them twice).
  // `joinM`: what counts as one stack at that zoom; `near`: what was already
  // that close when it opened (a neighbour never "joins").
  type FanState = { source: 'assets' | 'trail-heads'; ids: string[]; props: Map<string, Record<string, unknown>>; pos: Map<string, [number, number]>; side: 1 | -1; openZ: number; joinM: number; near: Set<string> }
  ...
  const fanMoveRef = useRef(() => {
    const f = fanRef.current
    const m = map.current
    if (f && m && Math.floor(m.getZoom()) !== f.openZ) { closeFanRef.current?.(); return }
    drawFanRef.current?.()
  })
  // The last features each stacking source was fed, so an opening fan knows
  // what is already parked around it.
  const stackFeaturesRef = useRef<{ assets: GeoJSON.Feature[]; 'trail-heads': GeoJSON.Feature[] }>({ assets: [], 'trail-heads': [] })
  ...
  /** Share this screen (Brian, Sep 19): the snapshot is taken as the sheet opens. */
  const [shareSnap, setShareSnap] = useState<SharedView | null>(null)
  ...
  const [exportRes, setExportRes] = useState<
    {
      title: string; imageUrl?: string | null; how?: DeliveryHow | null; error?: string | null; filename?: string
      /** The file as an object URL (a browser's direct Save) and its storage link (the app's). */
      blobUrl?: string | null; link?: ExportLink | null; linkState?: LinkState; linkError?: string | null
    } | null
  >(null)
  ...
  makePdfRef.current = async () => {
    ...
    try {
      ...
      const { how, blob } = await pdf.finish(file)
  ...
  /** The map as a picture — a link inside the app, a straight download in a
   *  browser, and a long press on the picture for Android's own save menu. */
  const makePng = async () => {
    const m = map.current
    if (!m) return
    try {
  ...
  // The tools those badges count, and the carrier each rides — drawn as the
  // carrier's badge, never as a dot of their own on top of it (and never a
  // stack member), whenever that carrier is itself on the map.
  const ridingCarrier = useMemo(() => {
    const out = new Map<string, string>()
    for (const [carrier, list] of Object.entries(aboard ?? {})) for (const t of list) if (t.settled) out.set(t.id, carrier)
    return out
  }, [aboard])
  ...
  // "This screen", for a teammate (Brian, Sep 19; migration 113): the
  // snapshot a saved view keeps, plus the camera, the time range and
  // playhead, what is followed or open, and the division filter.
  const buildSharedView = useCallback((): SharedView | null => {
    const m = map.current
    if (!m) return null
    const c = m.getCenter()
    const view: SharedView = {
      v: 1,
      cfg: currentViewCfg(),
      cam: { lng: c.lng, lat: c.lat, zoom: m.getZoom(), bearing: m.getBearing(), pitch: m.getPitch() },
      range,
    }
    if (range === 'custom') { view.from = customFrom; view.to = customTo }
    if (range !== 'live') view.t = pbT
    if (followId) view.follow = followId
    if (selectedAsset) view.asset = selectedAsset.id
    if (selectedZone) view.zone = selectedZone.id
    if (divisionFilter) view.division = divisionFilter
    if (!showLabels) view.labels = false
    if (sunMode) view.sun = true
    if (Object.keys(overlayOpacity).length) view.opacity = overlayOpacity
    return view
  }, [currentViewCfg, range, customFrom, customTo, pbT, followId, selectedAsset, selectedZone, divisionFilter, showLabels, sunMode, overlayOpacity])

  /** What a recipient will see, in plain words — the chips on the share sheet. */
  const shareSummary = useCallback((v: SharedView): { chips: string[]; title: string } => {
    const chips: string[] = []
    const baseLabel = BASEMAPS.find((b) => b.id === v.cfg.base)?.label ?? v.cfg.base
    chips.push(baseLabel)
    if (v.cfg.threeD || v.cfg.terrain) chips.push('3D')
    if (v.cfg.trailMode === 'trails') chips.push('Trails')
    else if (v.cfg.trailMode === 'heatmap') chips.push('Heat map')
    else if (v.cfg.trailMode === '3d') chips.push('3D activity')
    if (v.cfg.radar) chips.push('Radar')
    if (v.cfg.precip) chips.push('Rain totals')
    const n = Object.keys(v.cfg.overlays).length
    if (n) chips.push(`${n} layer${n === 1 ? '' : 's'}`)
    if (v.cfg.parcels) chips.push('Parcels')
    if (!v.cfg.zones) chips.push('Zones off')
    const rangeLabel = v.range === 'custom' ? 'Custom range' : (RANGES.find((r) => r.key === v.range)?.label ?? v.range)
    chips.push(rangeLabel)
    let subject: string | null = null
    if (v.follow) {
      subject = v.follow.startsWith('zone:')
        ? geofences.find((g) => g.id === v.follow!.slice(5))?.name ?? null
        : assets.find((a) => a.id === v.follow)?.name ?? null
      if (subject) chips.push(`Following ${subject}`)
    } else if (v.asset) {
      subject = assets.find((a) => a.id === v.asset)?.name ?? null
      if (subject) chips.push(subject)
    } else if (v.zone) {
      subject = geofences.find((g) => g.id === v.zone)?.name ?? null
      if (subject) chips.push(subject)
    }
    chips.push(`Zoom ${Math.round(v.cam.zoom)}`)
    return { chips, title: defaultViewTitle({ range: rangeLabel, subject, base: baseLabel }) }
  }, [assets, geofences])
...
map.current.addControl(new maplibregl.AttributionControl({ compact: true }), 'bottom-right')
    ...
    const initialSetup = () => {
      ...
      m.addLayer({
        ...
        // Sparse hops render on their own dashed layer below — solid ink
        // here would draw the very chords this split exists to kill.
        filter: ['!=', ['get', 'sparse'], 1],
      ...
      // Dashed "went roughly this way" connectors for sparse hops (Brian,
      // Aug 30: default-config trackers reporting every 5-15 min drew
      // 5-mile straight chords). Same source; the honest read is a faint
      // dotted line between the two real fixes, clearly not a road.
      m.addLayer({
        id: 'trails-sparse', type: 'line', source: 'trails',
        filter: ['==', ['get', 'sparse'], 1],
        layout: { 'line-cap': 'round', visibility: 'none' },
        paint: {
          'line-color': ['get', 'color'],
          'line-width': ['case', ['==', ['get', 'sel'], 1], 2.5, 1.8],
          'line-opacity': ['case', ['==', ['get', 'sel'], 1], 0.65, ['==', ['get', 'dim'], 1], 0.12, 0.45],
      ...
      m.on('click', 'place-pins', (e) => {
        // A count circle or an open stack drawn over the pin takes the tap.
        if (STACK_TAP.has(topMarkerAt(e.point) ?? '')) return
      ...
      const onStackTap = (layer: 'clusters' | 'head-clusters', source: 'assets' | 'trail-heads') => (e: maplibregl.MapLayerMouseEvent) => {
        ...
        Promise.all([src.getClusterLeaves(clusterId, Math.min(Math.max(total, 1), 500), 0), src.getClusterExpansionZoom(clusterId)])
          .then(
            ([leaves, zoom]) => openStackRef.current?.(source, at, (leaves ?? []) as GeoJSON.Feature[], total, zoom ?? m.getZoom() + 2),
            // A cluster from a frame ago (playback re-clusters every frame).
            () => m.easeTo({ center: at, zoom: m.getZoom() + 2, duration: 600 }))
      }
      ...
      m.addLayer({
        ...
        paint: {
          ...
          // Red when a member wears a live alert — opening the stack must not hide it.
          'circle-stroke-width': 2, 'circle-stroke-color': ['case', ['==', ['get', 'alert'], 1], '#fb5d5d', '#ff9e16'],
        },
      })
      m.addLayer({
        ...
        paint: { 'text-color': ['case', ['==', ['get', 'alert'], 1], '#fb5d5d', '#ff9e16'] },
      })
      m.addLayer({
        ...
        paint: {
          ...
          'circle-stroke-width': ['case', ['==', ['get', 'sel'], 1], 3, ['==', ['coalesce', ['get', 'alert'], 0], 1], 3, 2.5],
          'circle-stroke-color': ['case', ['==', ['get', 'sel'], 1], '#ffffff', ['==', ['coalesce', ['get', 'alert'], 0], 1], '#fb5d5d', '#04121d'],
        },
      })
      m.addLayer({
      ...
      m.on('click', (e) => {
        ...
        // With trails on the heads are the markers; the live dots' glow still
      ...
      if (!openedFromSaved) {
        ...
        // or a teammate's shared view (/map?v=) owns the opening frame instead.
        const urlSpot = typeof window !== 'undefined' && (
          (/[?&]lat=/.test(window.location.search) && /[?&]lng=/.test(window.location.search)) ||
  ...
  useEffect(() => {
    ...
    if (range === 'live' && trailMode === 'off') {
      // Same visibility rules as the dots: a type filtered off the map must
      // not be lassoed (a ring around invisible members reads as a bug).
      const shown = visible.filter((a) => filter.has(a.type))
      const groups = detectConvoys(shown)
      convoysRef.current = groups
      convoySrc?.setData(convoyRingGeoJSON(groups, shown))
    } else {
      convoysRef.current = []
      convoySrc?.setData({ type: 'FeatureCollection', features: [] })
    }
  }, [mapReady, assets, filter, isolateId, toolCounts, alertAssetIds, range, trailMode, tracksEff, selectedAsset, ridingCarrier])
  ...
  const updateMovementSources = useCallback((t: number) => {
    ...
    // Tools riding a carrier at this moment (tool → carrier) are that
    // carrier's badge — never a second marker on top of it, never a stack
    // member — as long as the carrier's own head is drawn.
    let aboardNow: Map<string, string> = ridingCarrierRef.current
      ...
      if (win) {
        ...
        for (const ep of episodesRef.current ?? []) {
          ...
          if (ep.startMs <= ts && (end == null || end >= ts)) {
            ...
            aboardNow.set(ep.member, ep.carrier)
          }
        }
      }
    }
  ...
  drawFanRef.current = () => {
    ...
    const alert = f.ids.some((id) => Number(f.props.get(id)?.alert) === 1) ? 1 : 0
    features.push({ type: 'Feature', geometry: { type: 'Point', coordinates: c }, properties: { kind: 'hub', n: f.ids.length, alert } })
  ...
  const stackSafeArea = (m: maplibregl.Map) => {
    ...
    // A card pinned over the top of the map (the Today tray — a quarter of a
    // phone screen) pushes the clear area down while it is open; a sheet up
    // from the bottom (an asset's panel on a phone) pushes it up.
    try {
      const box = el.getBoundingClientRect()
      document.querySelectorAll<HTMLElement>('[data-map-overlay="top"]').forEach((o) => {
        const r = o.getBoundingClientRect()
        if (r.height > 0 && r.bottom > box.top && r.top < box.top + h / 2) pad.top = Math.max(pad.top, Math.round(r.bottom - box.top) + 12)
      })
      document.querySelectorAll<HTMLElement>('[data-map-overlay="bottom"]').forEach((o) => {
        const r = o.getBoundingClientRect()
        if (r.height > 0 && r.top < box.bottom && r.bottom > box.top + h / 2) pad.bottom = Math.max(pad.bottom, Math.round(box.bottom - r.top) + 12)
      })
  ...
  const openFan = (source: 'assets' | 'trail-heads', leaves: GeoJSON.Feature[]) => {
    ...
    const memberPts = ids.map((id) => ({ id, lng: pos.get(id)![0], lat: pos.get(id)![1] }))
    const mid = centroidOf(memberPts)
    const side: 1 | -1 = mid && m.project(mid).x > m.getContainer().clientWidth / 2 ? -1 : 1
    const openZ = Math.floor(m.getZoom())
    const joinM = stackRadiusMetres(openZ, mid?.[1] ?? 0)
    const others: StackPoint[] = []
    for (const ft of stackFeaturesRef.current[source]) {
      const fid = ft.properties?.id
      const c = (ft.geometry as GeoJSON.Point | undefined)?.coordinates
      if (fid == null || !c || pos.has(String(fid))) continue
      others.push({ id: String(fid), lng: c[0], lat: c[1] })
    }
    fanRef.current = { source, ids, props, pos, side, openZ, joinM, near: nearbyIds(memberPts, others, joinM) }
  ...
  openStackRef.current = (source, at, leaves, total, expansionZoom) => {
    ...
    closeFanRef.current?.()
    setStackPeek(null)
    // A circle took this tap: a zone, place or device sheet it sat over goes.
    setSelectedZone(null)
    setSelectedDevice(null)
    setSelectedPlace(null)
  ...
  syncFanRef.current = (features) => {
    ...
    if (members.length !== f.ids.length || !fanStillHolds(members, others, f.joinM, f.near)) { closeFanRef.current?.(); return }
    drawFanRef.current?.()
  }
  ...
  useEffect(() => {
    ...
    // Once, when the camera has settled and the new zoom's circles are drawn:
    // on idle, or a timer that keeps checking (the map may already be idle —
    // or never idle, with the pulse animating). Only once: a fan you closed
    // must not pop back open on a later idle.
    let done = false
    let t = 0
    const run = () => {
      if (done || cancelled) return
      window.clearTimeout(t)
      if (m.isMoving() || !m.areTilesLoaded()) { t = window.setTimeout(run, 400); return }
      done = true
      m.off('idle', run)
      void reveal()
    }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        // Prune the shown-position cache to aircraft still in the feed — a
  ...
  // Open a teammate's shared view (/map?v=<id>, migration 113): layers,
  // camera, range + playhead, follow / selection, division — once, when the
  // map is ready. Then the recipient owns the map; a shared view is a
  // starting point exactly like a saved one.
  const sharedViewAppliedRef = useRef(false)
  useEffect(() => {
    if (!mapReady || kiosk || !sharedView || sharedViewAppliedRef.current) return
    sharedViewAppliedRef.current = true
    const v = sharedView.view
    applyView({ id: `shared-${sharedView.id}`, name: sharedView.title ?? 'Shared view', cfg: v.cfg })
    ...
    if (v.range !== 'live') {
      ...
      handleRange(v.range)
      if (typeof v.t === 'number') { tRef.current = v.t; setPbT(v.t) }
    }
    ...
    attempt()
    // Drop ?v= once applied — a refresh should behave like a normal map
    // open, not snap back to the shared frame (same rule as ?m= and ?follow=).
    try {
      const u = new URL(window.location.href)
    ...
    toast(`${sharedView.from ?? 'A teammate'} shared this view${sharedView.title ? `: ${sharedView.title}` : ''}`, { ttl: 5000 })
    ...
    <div className={'relative w-full h-full bg-navy-950' + (kiosk ? ' kiosk-map' : ' map-live')}>
      ...
      <ExportMenu
        ...
        onPick={(kind) => {
          ...
          if (kind === 'link') {
            const snap = buildSharedView()
            if (!snap) { toast('The map is still loading — try again in a second.', { variant: 'error' }); return }
            setShareSnap(snap)
            return
          }
          setExportBusy(kind)
      ...
      <ExportResult
        ...
        link={exportRes?.link}
        linkState={exportRes?.linkState ?? 'off'}
        linkError={exportRes?.linkError}
      />
      ...
      {shareSnap && (() => {
        const sum = shareSummary(shareSnap)
        return (
          <ShareViewSheet
            open
            onClose={() => setShareSnap(null)}
            snapshot={shareSnap}
            summary={sum.chips}
            defaultTitle={sum.title}
          />
        )
      })()}
      ...
      {searchedPin && !navDest && (
        <div className="absolute inset-x-3 md:inset-x-auto md:right-4 md:w-[360px] bottom-[132px] md:bottom-auto md:top-16 z-30 rounded-xl border border-navy-700 bg-navy-950/95 backdrop-blur px-3 py-2.5 shadow-panel flex items-center gap-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            {searchedPin.err
              ? <p className="text-[11px] text-alert truncate">{searchedPin.err}</p>
          ...
          {!isMock && (
            <button
              onClick={() => {
                createPlaceAction({ name: searchedPin.name, kind: 'other', lat: searchedPin.lat, lng: searchedPin.lng, address: searchedPin.sub || null }).then((res) => {
                  if (res.ok && res.place) { onPlacesChanged?.([...placesRef.current, res.place]); setSearchedPin(null); setSelectedPlace(res.place) }
                  // A refusal (viewer role, demo) must say so — a Save that
                  // silently does nothing reads as saved (ship-check).
                  else setSearchedPin((sp) => (sp ? { ...sp, err: res.error ?? 'Could not save that place.' } : sp))
                }).catch(() => setSearchedPin((sp) => (sp ? { ...sp, err: 'Could not save — check your signal and try again.' } : sp)))
              }}
