...
const WAIVED = [
  ...
  /(?:plaintiff|mortgagee)(?:(?!\bnot\b)[^.;]){0,80}(?:waive[sd]?|has waived|does not (?:seek|demand|request)|will not (?:seek|demand))[^.;]{0,40}deficiency/i,
  /waiver of (?:any |the |its )?(?:right to a )?deficiency/i, /deficiency waived/i,
  /bidding will not remain open|(?:deficiency[^.;]{0,80}|not remain open[^.;]{0,40})sale (?:is|shall be|will be) final/i,
]
const DEMANDED = [
  /(?<!no (?:personal or )?)deficiency judgment (?:(?:is|being|has been|will be)\s+){0,2}(?:demanded|requested|sought|preserved)/i,
  /deficiency (?:(?:is|being)\s+){0,2}(?:demanded|preserved)/i, /deficiency judgment (?:was|has been|having been|is) (?:granted|entered|awarded|reserved)/i, /(?:does|do|will) not waive[^.;]{0,40}deficiency/i,
  /(?<!not )remain open (?:after the (?:date of )?sale )?(?:for )?(?:a period of )?(?:thirty|30)/i,
...
export function deficiencyStatus(text) {
  ...
  for (const s of sentences) {
    ...
    if (/not waive/i.test(s)) { votes.demanded++; quotes.push('D: ' + s.trim().slice(0, 220)); continue }
...
export function parseNotice(text) {
  ...
  return {
    saleDate: pick(/(?:will sell|offer(?:ed)? for sale|will (?:be )?(?:sold|sell)|sale will be held)[\s\S]{0,100}?(?:[A-Za-z]+day,?\s+)?((?:January|February|March|April|May|June|July|August|September|October|November|December) \d{1,2},? \d{4})/i),
    ...
    reopenDate: pick(/(?:re-?open(?:ed)?|bidding will (?:re)?open)[^.]{0,60}?on (?:[A-Za-z]+day,?\s+)?([A-Za-z]+ \d{1,2},? \d{4})/i),
