-- 057: tool presence hours — BLE pucks bank on-site time via their carrier.
--
-- Approved Aug 6 ("#2 - yes, I want this") + "need tool time tracked and
-- shown in zones" (Aug 7). Tool tags have no GPS: a tool is on site exactly
-- when one of its pairing episodes (pairing_log, 021) overlaps its carrier's
-- session in the zone. Those overlaps become zone_sessions rows (the visit
-- feed shows the tool arriving/leaving with its truck) and usage_daily rows
-- with on_site_secs only — active_secs stays 0 FOREVER for presence rows;
-- presence is not engine time. Interim until TAT141s put real fixes on the
-- machines; a tool that gains its own GPS fixes in the window is skipped
-- here (GPS truth wins, no double counting).
--
-- Same idempotence contract as 056: rebuild deletes + recomputes its
-- zone/window, so cron / backfill / manual replay converge.
...
BEGIN
  ...
  -- ── Pass 2: TOOL PRESENCE (057) ───────────────────────────────────────────
  -- Pairing episode ∩ carrier's session in this zone, clipped to the window
  -- so every write lands inside the range the DELETEs cleared. Episodes
  -- still open use last_seen (the ingest keeps it fresh), never now().
  WITH tool_iv AS (
...
-- Re-bank the year so existing tools get their history immediately.
