...
- Zone imagery timeline + map overlays (migrations 052–053) — dated drone/site photos per zone with a date slider on the zone page (Brian's Mavic Air 2 flies daily); evidence locker for closeout/pay apps. "Place on map" pins a shot to ground corners (pan/size/rotate against satellite base) → latest placed shot per zone renders on /map under the Site imagery layer toggle. Next: SkyFi order button; earthwork volumes = Dronelink/Litchi grid flights → WebODM ortho+DSM upload → DSM differencing (Mavic Air 2 has no waypoint missions in DJI Fly; raw JPEGs alone can't measure volume)
