...
- **Customer #2 (Sep 9 2026):** the sister paving company (Charleston) took two FMM00As (…33818, …48247), one TAT141 (…4734) and three Eye Beacons (MACs …B56B/…B573/…B521). Those trackers were detached from the DCG placeholders into DCG's drawer (tracker_moves rows say why). Transfer plan: owner registers at /register (his user id becomes the company id = Master), then move the registry rows (093: one registry per IMEI) + create the assets in his company. Beacons register by printed MAC. **Never put a customer's people's names in the app or the repo — company names only.**
...
| **Apple Developer** | 🔴 **ORG ENROLLMENT DENIED as final (phone, Sep 4 — Megan)** → going **Individual** | Enrollment N37H75H2FX, case **20000149520723**. Apple would not say what failed; the denial email is still to be read (devprograms@apple.com is the appeal address; org conversion possible later once D&B matches HAMMERTRACK LLC verbatim). **Personal route (task #51):** Apple Developer app on the iPhone → Enroll → Individual → license scan + selfie → $99 on the Mercury card. Seller name becomes "Brian Dillard". When approved: App Store Connect API key (Issuer ID, Key ID, .p8) + Team ID → `ios-release` workflow to TestFlight. |
